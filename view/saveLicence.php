<?php 
	...
	$e_tmp=$_FILES['rL1']['tmp_name'];
	$f_tmp=$_FILES['rL2']['tmp_name'];
    ...
    $numAdd4 = $numMax + 1;
	$extValide4 = array('pdf','png','jpeg','gif' );
	$extentionUpload4 = strtolower(substr(strrchr($e, '.'), 1));
	in_array($extentionUpload4, $extValide4);
	$chemin = "../public/bootstrap4/docs/releve/l1/".$numAdd4.".".$extentionUpload4;
	$resultat = move_uploaded_file($e_tmp, $chemin);
	$rL1 = $numAdd4.".".$extentionUpload4;

    $numAdd5 = $numMax + 1;
	$extValide5 = array('pdf','png','jpeg','gif' );
	$extentionUpload5 = strtolower(substr(strrchr($f, '.'), 1));
	in_array($extentionUpload5, $extValide5);
	$chemin = "../public/bootstrap4/docs/releve/l2/".$numAdd5.".".$extentionUpload5;
	$resultat = move_uploaded_file($f_tmp, $chemin);
	$rL2 = $numAdd5.".".$extentionUpload5;

	$query2 = $bd->prepare("SELECT * FROM commande  WHERE typeDiplome = ? AND idEtud = ?");
	$query2->execute(array($licence,$idEtudiant));
	$don = $query2->fetch(PDO::FETCH_ASSOC);

	if ($don) {
		header("location:comDip.php?sms=1");
	}else{
		$req1 = $bd->prepare('INSERT INTO `istm_dip`.`commande` (`id`, `dateCommande`, `typeDiplome`, `borderau`, `rG1`, `rG2`, `rG3`, `rL1`, `rL2`, `statut`, `idEtud`, `idAgent`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)');
		$req1->execute(array($dateCom,$licence,$recu,$rG1,$rG2,$rG3,$rL1,$rL2,$statut,$idEtudiant));
